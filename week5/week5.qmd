...
data_binned <- sim_data %>% 
  mutate(distance_bin = cut(distance, breaks = seq(-20, 20, by = 1), include.lowest = TRUE)) %>% # include.lowest ensures 20 is in there
...
rdd_lm <- ggplot(data_binned, aes(x = avg_distance, y = avg_deforest, color = as.factor(protected))) +
  ...
  geom_smooth(method = "lm", aes(group = protected)) + # separate lines for both protected and not protected
...
**RESPONSE**: The two lines represent deforestation rate relative to distance to the protected area border (distance = 0 represents the boundary). If there was no discontinuity at distance = 0, it would suggest that the presence of a protected area has no effect on deforestation rate (aka, no treatment effect).
...
rdd_ols <- lm(
  deforest ~ # outcome
    ...
    protected*distance + # interaction term allows slopes to vary
    slope_cat + road_cat + water_cat + soil_cat, # covariates/controls
  data = sim_data
)
...
**RESPONSE**: Estimated causal effect of protected area on deforestation is -0.1. This represents that the presence of the treatment, or protected area status, decreases deforestation rate by 0.1 (10%). This does match the plot. The interaction coefficient represents the difference in slope in protected areas compared to that of unprotected areas (based on distance).
...
**Kernel** (Triangular): Gives higher weight to data points close to the cutoff (values closer to 0 -- the boundary -- have a higher weight).
...
global_samp <- sim_data %>%
     sample_n(size = nrow(sim_data) * 1.0) # <<< e.g., .5 for 50% of the data (to speed things up)
```
...
**RESPONSE**: This estimate is most likely more accurate, because it weighs the observations that are most similar to each other more than those that are farther apart. Consideration of covariates is more accurate. Specified cutoff; and also used a specific bandwidth (rdrobust calculated the best bandwidth).
...
**RESPONSE**: The rdpoly plot predicts a curved line, that perhaps is better representative of the true data distribution. The curvature also conveys the difference in RATE of effect of distance to protected area, instead of it being a constant slope before and after the cutoff. Specifically, decrease in deforestation rate is steeper closer to the cutoff.
...
run_country_rdd <- function(country_name){
  ...
  # Step 1: pick data only from the country that is specified in function
  df_country <- sim_data %>% filter(country == country_name)
  
  # Step 2: run model based on data just for specified country
  rdd_model <- rdrobust(
...
rdd_6country <- lapply(unique(sim_data$country), run_country_rdd) # apply to unique values in country column

# Print summary for "DRC" (use`unique(sim_data$country)` to figure out which order it is in)
...
**RESPONSE**:
The patterns of these results are similar to that of Neal 2024! Though there is more discontinuity for certain countries (like Indonesia and Brazil). Faceting by country reveals different relationships than when it is grouped by the entire country, which makes sense because different countries may enforce protected areas differently.