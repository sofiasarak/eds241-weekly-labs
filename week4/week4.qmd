...
*Response:* `dist_mangroves` and `reef_exposure`.
...
*Response:* Differences in reef exposure can affect fish biodiversity and population due to differences in wave energy. For example, exposed reefs could potentially be receiving more nutrients, leading to more fish biomass in MPA sites.
...
*Response:* The Average Treatment effect on the Treated represents the effect of MPA status on fish biomass, *only* in treated sites. In other words, how fish biomass would change if our MPA sites were not MPAs?
...
match_model <- matchit(
  ...
  # specify all variables that we want matched
  treated_mpa ~ dist_to_deep_water + ssta_freq + reef_exposure + 
...
# weights column represents how many times a control site was used to match with a treatment site
```
...
# data frames
# 1. original balance summary
# 2. new balance summary
# 3. sample size of each
```
...
*Response:* 108, or all, of the treated units were matched, and 37 control sites were included.

Note: When I ran this code, my number of control sites was 33. I `set.seed()` and everything...not sure what went wrong!

**Q6.** Find the covariate with the largest standardized mean difference (SMD) *after matching*. Which covariate is it?

*Response:* `dist_to_deep_water` has the largest SMD, of 0.3073. Balance became worse for this covariate after matching.
...
reg_ecological <- lm(biomass_ecological ~ treated_mpa,
                     data = matched_data,
                     weights = weights) # tell model to use weights column of our data frame
...
*Response:* In the fisheries model, MPA treatment increases fish biomass by an average of ~111 kg/hectare.

Note: Again, my output doesn't exactly match what Annie/my partner got -- I'm a few numbers off, unsure why.

**Q8.** What is the biggest remaining threat to causal interpretation of regression coefficients after matching?

*Response:* Really large standard error, meaning measurement is very imprecise. This suggests that there are covariates that we haven't observed. The matching method does not address unobserved selection bias.
...
*Response:* `vpd` coefficient increases from the no-FE to FE model. This suggests that not controlling for plot/time differences was obscuring the entire true effect of `vpd`; our fixed effects (plot/year) *were* confounding our model.
...
*Response:* The presence of an El Niño event or a pest outbreak from one year to the next could bias estimates.
...
*Response:* Sediment nutrients, plot density, or proximity to certain land forms could bias estimates if not controlled for.
