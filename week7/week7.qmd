...
**RESPONSE:** Picking a single state would most likely mean that we are more closely synthetically reproducing Texas than California. The purpose of picking multiple states is that we can optimize our synthetic control to be most representative of California. Picking just Texas could confound results.
...
**RESPONSE:** We want the synthetic to track closely because we want it to be representative of our treated state. If it doesn't track well, that could mean that we can't conclude causal results.
...
**RESPONSE:** Including CO2 as a predictor ensures that CO2 will be similar. This is necessary because it is possible to be similar based on all of the other predictors but still drastically differ from CO2, or the outcome.
...
**RESPONSE:** 49 states and 22 years (from 2000 to 2022).
...
scm_data <- data %>%
  ...
  synthetic_control(
    ...
    i_unit = "CA", # treated year and unit; "i" stands for intervention
    i_time = 2013,
    
    # generate placebo tests for all states; aka, synthetic for all states 
    generate_placebos = TRUE) %>%
  
  # define the pre_treatment characteristics and use to match synthetic to control
  generate_predictor(  
    time_window = 2000:2012, # pre-treatment period only to compute predictors; find matches only for this period
    ...
    co2_pre_mean = mean(co2_power_pc_tons, na.rm = TRUE)) %>% # average pre-treatment outcome
  # at this stage, our predictors contain the averages of these variables (as columns, for example)
  
  # choose donor weights 
  # take the predictor summaries from above and actually figure out how much wight to give each donor state 
  generate_weights(
  ...
  # constructing synthetic california using optimized donor weights 
  generate_control() 
```
...
**RESPONSE:** The synthetic California is a relatively good control -- the slope follows the same pattern, though the peaks don't match up.

2.  What happens to the gap between the two lines after 2013? What does this suggest about the effect of cap-and-trade?

**RESPONSE:** Gap shows differnce, cap and trade effective in 
...
scm_data %>% 
  plot_placebos() +
  labs(title = "Placebo Tests, aka Gaps Between Placebos")
```
...
Yes! Gap increases post-treatment.
...
**RESPONSE:** California ranks the highest in the MSPE ratio distribution. 
...
scm_data %>% 
  grab_balance_table()
```
Interpretation: optimizer had a harder time finding matches in certain variables instead of others; hard to find optimized synthetic California that matched on all 4. We just want values between synthetic California and real CA to be close to each other.
...
**RESPONSE:** Synthetic California is closer to actual California than the donor sample, telling us that the weights did what they were supposed to do.
...
**RESPONSE:** The cap-and-trade was successful in reducing emissions; California's emissions fall below synthetic California's.
...
**RESPONSE:** strong pre-treatment fit: yes!
transparent donor pool: if treated states excluded, then yes!
no spillover: no, there could be spillover
favorable placebo comparison: yes (balance table)
...
**RESPONSE:** It would make treatment effect inaccurate.
...
**RESPONSE:** Pre-treatment outcome is included in synthetic control method, whereas with matching, only other predictors were used.
