...
weekly_la_phx <- did_data_az_ca %>% 
  mutate(
    date_obj = mdy(date),
    week = floor_date(date_obj, unit = "week")) %>%  # figures out week that obv is in, and then round down to that Sunday
  
  # filter for the year and month we want
  filter(year == 2020, month(date_obj) %in% c(2, 3, 4)) %>% 
  
  # group by week and city
  group_by(week, city) %>% 
  
  # mean no2 per week and city
  summarize(mean_no2 = mean(daily_max_1_hour_no2_concentration, na.rm = TRUE),
            .groups = "drop")
...
did_data_nm_tx <- did_data_nm_tx %>% 
  mutate(
    date_obj = mdy(date), # making date into a date object
...
```{r}
# getting average no2 for obvs that were in nm after lockdown
y_trt_post <- did_data_nm_tx %>% 
  ...
  # pulls actual value out (instead of saving it with a column name, etc)
  pull()

# getting average no2 for obvs that were in tx after lockdown
y_cont_post <- did_data_nm_tx %>% 
  filter(treated == 0,
         is_post == 1,
         year == 2020,
         month(date_obj) %in% c(2, 3, 4)) %>% 
...
# getting average no2 for obvs that were in nm pre lockdown
...
# getting average no2 for obvs that were in tx pre lockdown
y_cont_pre <- did_data_nm_tx %>% 
  filter(treated == 0,
         is_post == 0,
         year == 2020,
         month(date_obj) %in% c(2, 3, 4)) %>% 
...
trt_effect <- (y_trt_post - y_trt_pre) - (y_cont_post - y_cont_pre)
...
DiD coeff: treated:is_post
coeff: is only relative to lockdown and only in this time frame, not comparing NM and TX on average
...
Our model controls for seasonal trends because TX and NM should be experiencing similar seasonal patterns.
...
**Answer:** Because we see parallel trends between Texas and New Mexico before the treatment, Texas makes a compelling counterfactual. But, trends do diverge right before the lockdown, so it is not perfect. The rest of the data also fluctuates a lot -- here, we are only dealing with three months of data. 
...
**Answer:** The `treat:isposs` coefficient represents the Treatment Effect. The coefficient is not statistically significant. There is 22% probablity that we would observe these results if the lockdown had not happened.
...
**Answer:** Counterfactual: El Paso; dust storm would not effect DiD estimate because counties are right next to each other.
...
**Answer:** Our DiD estimator would get smaller, because there would be a smaller difference in emissions. Our control group is doing what we expected our treated group to do.
