...
```
Intercept (77.91): Without any treatment, we expect the number of trash piles to be ~78 on average.
...
```
**To calculate percent change:** (exp(coefficient) - 1) * 100

Intercept (78.26): On average, the model estimates our control group to have ~78 waste piles.

Treatment (-32%): On average, there are 32% fewer trash piles in the treatment group than in the control group.
...
Dispersion ratio rule of thumb is below 3. Dispersion ratio = 1, exact match between mean and variance.
...
```{r}
m3_log <- lm(formula = log(waste_piles) ~ treat, # Applied log because we want percent change (non-negative)
             data = post_treat_subset)
...
Maps model-predicted distributions and also plots observed data to see if their shapes are similar.
...
Change OLS so it can be interpreted in percent change.
...
Before the treatment even started, non-treatment neighborhoods already had a lower waste pile count.
...
```{r}
m4_control <- lm(
  formula = log(waste_piles) ~ treat + log(pre_count_avg),
  data = post_treat_subset
)

summ(m4_control)
```
intercept: 1.6; number of waste piles when the control is applied and pre-count waste pile average is 0.
treat: -23.6 %; reduction in waste piles when treatment is applied, controlling for pre-count waste pile average.
log(pre_count_avg): .85% increase in post treat waste piles (don't need to convert because log() to log()); control vars are not typically interpreted 

Let's compare our two log OLS Regression Models. 

```{r}
export_summs(m3_log, m4_control,
             model.names = c("Log OLS Regression", "Log OLS Regression with control var"),
             statistics = "none")
```
Treatment SE is smaller when including control variable.
