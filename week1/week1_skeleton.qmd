...
balance_summary <- class_data %>% 
  gtsummary::tbl_summary(                   # gtsummary SPECIFICALLY makes statistical tables
...
tab_spanner(
  ...
  columns = c(stat_1, stat_2)     # Specifies spanner to span just the two statistics; could specify everything()
) %>% 
  
  # Change column labels
cols_label(
  label = "Variable",
  p.value = "P-Value"            # Could change control and treatment labels using "stat_1" and "stat_2"
) %>% 
...
tbl_2way <- lobsters %>% 
  ...
  adorn_totals("row")          # "row" does it by row, "col" by column
```
...
```{r}
tbl_2way %>% 
  gt(rowname_col = "year") %>%          # coming from a data frame, use gt(); from a gt summary table, use as_gt()
  tab_header(
...
fancy_table <- tbl_3way %>% 
  ...
  # Tell gt that our row name column comes from "year" in our original data frame
  gt(rowname_col = "year") %>%          # coming from a data frame, use gt(); from a gt summary table, use as_gt()
  ...
  # Create region name spanner for both control and treatment columns
  tab_spanner(
    label = "Bolærne",
  ...
  # Change individual column names to be cleaner
  cols_label(
    bol_con = "Control",
    bol_mpa = "Treatment",
    flo_con = "Control",
...
## HW 1 backup code
```{r}
# NOTE TO GRADER: the `gtsummary::tbl_summary()` method no longer works for my version of R/the gt summary package. Originally, I had successfully used the code below to make my summary table, but after revisiting this assignment at a later date, I kept getting (truly unfixable) errors regarding the way I was specifying the `statistic` argument. 

# After some research online, it seems as though the problem comes from the package itself -- perhaps it had updated between the first and second time I opened this assignment. I have included my original code below to verify that I do know how to use this method, but I had to workaround the issue and use group_by() and summarize() to get a similar result.

# Optimize df for analysis
spiny_counts <- spiny_counts %>% 
  drop_na(counts, mean_size_mm) %>% 
  mutate(counts = as.numeric(counts))

# Create initial summary table
lobster_summary <- spiny_counts %>% 
  group_by(mpa) %>% 
  summarize(counts = round(mean(counts, na.rm = TRUE), 2),
            size_mm = round(mean(mean_size_mm, na.rm = TRUE), 2)) %>% 
  gt() # Save as gt object
```
